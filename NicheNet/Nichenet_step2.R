...
# For less expanded to more expanded bystanders ---------------------------
srat = readRDS('srat_subset_for_DEG_bystanders.rds')
bg_filename = 'expressed_genes_in_less_expanded.csv'
deg_filename = 'expansion_bystanders_DEG.csv'
...
# Same but for less expanded to mtb-specific 1 ----------------------------
srat = readRDS('srat_subset_for_DEG_mtb1.rds')
bg_filename = 'expressed_genes_in_less_expanded.csv'
deg_filename = 'expansion_DEG_mtb1.csv'
...
DE_table_receiver = read.table(deg_filename, sep = ',', header = 1, row.names = 1)
geneset_oi <- DE_table_receiver %>% filter(q_value <= 0.05 & normalized_effect >= 1) %>% pull(gene_short_name)
geneset_oi <- geneset_oi %>% .[. %in% rownames(ligand_target_matrix)]
ligand_activities <- predict_ligand_activities(geneset = geneset_oi,
                                               background_expressed_genes = background_expressed_genes$X0,
                                               ligand_target_matrix = ligand_target_matrix,
...
sorted_table <- ligand_activities[order(ligand_activities$auroc,decreasing = TRUE), ]
write.table(sorted_table, paste0('nichenet_result_mtb1.csv'), row.names = FALSE)
...
all_targets <- unique(as.vector(as.matrix(target_gene_union)))
write.table(all_targets, 'nichenet_targets_mtb1.csv', row.names = FALSE)  
...
dotplot = DotPlot(subset_srat, features = all_targets, assay = 'RNA') +
  RotatedAxis() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
ggsave("downstream_in_more_dotplot_mtb1.jpg", plot = dotplot, width = 12, height = 4,dpi = 300)
...
htmap = pheatmap(t(normalized_power_mat_to_plot),# Scale by row (optional, for better visualization)
                 ...
                 show_rownames = TRUE,
                 show_colnames = TRUE)
ggsave("ligand_powermap_mtb1.jpg", plot = htmap, width = 12, height = 5,dpi = 300)
